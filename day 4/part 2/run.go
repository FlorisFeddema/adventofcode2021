...
func playgame(rolls []string, cards []bingo) (bingo, []string) {
	...
	for _, e := range rolls {
		...
		if len(cards) == 1 {
			if checkHorizontal(cards[0], currentRolls) || checkVertical(cards[0], currentRolls) {
				return cards[0], currentRolls
			}
		}

		for _, f := range cards {
			if !checkHorizontal(f, currentRolls) && !checkVertical(f, currentRolls) {
				currentCards = append(currentCards, f)
			}
		}
		cards = make([]bingo, len(currentCards))
		copy(cards, currentCards)
	}
